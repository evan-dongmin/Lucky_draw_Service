...
>   바꿀지" 판단할 때 여기부터 보세요.
> - **`타추위-추첨프로그램_운영자매뉴얼.md`** — **행사 당일 진행자용.** 설정
>   4단계, 권장 진행 시간(300초)과 구간 배분, 참가자 안내 문구, 문제 발생 시 대처.
> - `타추위-추첨프로그램_게임소개.md` — 참가자용 쉬운 설명.
...
- **무대 화면(전체화면 레이싱 중계)**: 부서 색상 카트가 **직선 구간을
  ...
  **8종 장애물**(콘🚧·기름통🛢️·타이어🛞·바나나🍌·웅덩이💧·바위🪨·폭탄💣·얼음🧊,
  종류마다 크기·움직임 패턴이 다르고 바위는 카트의 약 2배 크기)이
  **결승선까지 가는 구간에만 고른 간격으로** 놓이고 좌우로 흔들리며
  회전합니다(움직임까지 전부 시드에서 파생돼 관전 화면이 몇 대든 똑같이
  움직입니다). **부딪히면 실제로 순위가 밀립니다** — 카트가 장애물에
  닿는 순간부터 그 종류에 맞는 시간만큼 진행 속도가 실제로 떨어지고
  (폭탄·바위는 거의 멈춰 서고, 콘은 살짝 휘청하는 정도), 이 페널티가
  최종 순위 계산에도 그대로 반영됩니다. 다만 위치가 뒤로 가는 일은
  없고(속도만 느려질 뿐 항상 앞으로 나아갑니다), `progress_ratio=1.0`
  에서는 감속과 무관하게 항상 정확히 목표 순위의 자리에 도달하도록
  설계돼 있어 화면과 발표 결과가 어긋나지 않습니다. **결승선을 넘는
  것이 곧 통과·승리 기준**이며, 라운드·인원수와 무관하게 항상 트랙 끝
  가까이에 큼직한 체커기 게이트로 그려지고 그 너머에는 장애물이
  생기지 않습니다.
...
333개 테스트가 공정성 엔진의 결정론·위변조 탐지, 부서 통과율 계산,
