...
def test_kart_lane_and_obstacle_lane_share_the_same_axis():
    ...
    둘이 다른 축이면 "보이는 충돌"과 "판정된 충돌"이 영영 일치할 수 없다.
    맞는 장애물이 있는 카트는 반드시 그 장애물과 레인 번호가 같아야 한다."""
    from app import race as race_module
    ...
    for round_index in (1, 2, 3):
        ...
        for i in range(120):
            pid = f"P{i:04d}"
            hits = race_module.kart_hits(seed, pid, round_index)
            if not hits:
                continue
            kart_lane = race_module.lane_for(seed, pid, round_index)
            for hazard in hits:
                assert hazard["lane"] == kart_lane, (
                    f"{pid}: 카트 레인 {kart_lane} vs 맞은 장애물 레인 {hazard['lane']}"
                )
            checked += 1
        assert checked >= 5, f"R{round_index} 표본이 너무 적습니다"
