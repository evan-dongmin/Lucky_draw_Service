...
class MCAgent:
    ...
    def pregenerate(self, tags: list[str] | None = None, lines_per_tag: int = 6) -> None:
    ...
    def pick_line(self, tag: str, **params: Any) -> str:
        """같은 태그를 연달아 반복하지 않도록 순환(round-robin)하며 고른다."""
        pool = self.pool_for(tag)
        if not pool:
            return ""
        idx = self._rotation.get(tag, 0) % len(pool)
        self._rotation[tag] = idx + 1
        return render_line(pool[idx], **params)
